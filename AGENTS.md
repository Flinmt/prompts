...
- `Medium/`: geometric full-body character prompts; `Medium/Criaturas/` contains the creature variant.
...
Character files follow `Nome - Funcao.md`; when system instructions require it, omit accents from the function portion (for example, `Nome - Cacador de Monstros.md`). Use descriptive lowercase snake_case for templates and instruction files, such as `medium_creature_template.md`.
...
Recent history uses Conventional Commit-style subjects such as `feat(astrael): adota padrao geometrico de meio corpo` and `fix: remove json legado`. Use an imperative, concise subject with an optional system scope: `feat(medium): ...`, `fix(animale): ...`, or `docs: ...`.
